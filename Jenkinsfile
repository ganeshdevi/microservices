pipeline {
    ...
    stages {
        stage('SCM Checkout'){
            steps {
            git branch: 'main', url: 'https://github.com/ganeshdevi/microservices.git'
            sh 'ls'
            }
        }
                        ...
                        script{
                            dir('wishlist-microservice-python') {
                                ...
                                withSonarQubeEnv('sonar-pro') {
                                    ...
                                    -D sonar.host.url=http://52.77.240.68:9000/"""
                                }
                            }
                        }
                    }
                ...
                parallel (
                    'docker login': {
                        withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
                            sh "docker login -u iamdevi -p ${dockerPassword}"
                        }
                    },
                    'ui-web-app-reactjs': {
                        dir('ui-web-app-reactjs'){
                            sh """
                            docker build -t iamdevi/ui:v1 .
                            docker push iamdevi/ui:v1
                            docker rmi iamdevi/ui:v1
                            """
                        }
                    },
                    'zuul-api-gateway' : {
                        dir('zuul-api-gateway'){
                            sh """
                            docker build -t iamdevi/api:v1 .
                            docker push iamdevi/api:v1
                            docker rmi iamdevi/api:v1
                            """
                        }
                    },
                    'offers-microservice-spring-boot': {
                        dir('offers-microservice-spring-boot'){
                            sh """
                            docker build -t iamdevi/spring:v1 .
                            docker push iamdevi/spring:v1
                            docker rmi iamdevi/spring:v1
                            """
                        }
                    },
                    'shoes-microservice-spring-boot': {
                        dir('shoes-microservice-spring-boot'){
                            sh """
                            docker build -t iamdevi/spring:v2 .
                            docker push iamdevi/spring:v2
                            docker rmi iamdevi/spring:v2
                            """
                        }
                    },
                    'cart-microservice-nodejs': {
                        dir('cart-microservice-nodejs'){
                            sh """
                            docker build -t iamdevi/ui:v2 .
                            docker push iamdevi/ui:v2
                            docker rmi iamdevi/ui:v2
                            """
                        }
                    },
                    'wishlist-microservice-python': {
                        dir('wishlist-microservice-python'){
                            sh """
                            docker build -t iamdevi/python:v1 .
                            docker push iamdevi/python:v1
                            docker rmi iamdevi/python:v1
                            """
                        }
                    }
                )
